...
1. Install the module-boundary hook and export the env it needs (the
   `shinycov --setup` command writes a `sitecustomize.py` into `.shiny.cov/`
   and prints the `export` line to copy):

   ```bash
   shinycov --setup
   # export PYTHONPATH=".shiny.cov:$PYTHONPATH" SHINYCOV_OUTPUT_DIR=".shiny.cov" SHINYCOV_SOURCE=cypress
   eval "$(shinycov --setup)"
   ```

2. Launch the app under coverage measurement (so the app process is measured
...
3. Configure Cypress the same way as for R Shiny: add
...
4. Run Cypress, then collect and report with the standalone `shinycov` command
