...
def test_attach_modules_attributes_by_longest_prefix():
    controllers._modules.clear()
    controllers._modules.update({"app", "app-inner"})
    ...
    controllers._attach_modules(snapshot)
...
def test_attach_modules_noop_without_boundaries():
    snapshot = {
        "inputs": [{"id": "x"}],
        "outputs": [],
        "tabs": [],
...
def test_instrument_modules_records_namespace(monkeypatch):
    ...
    try:
        controllers._instrument_modules()

        # Entering a module UI context should record the resolved prefix.
        with shiny_module.namespace_context("outer"):
            with shiny_module.namespace_context("inner"):
                pass

        assert controllers.get_modules() == ["outer", "outer-inner"]
    finally:
        shiny_module.namespace_context = original
